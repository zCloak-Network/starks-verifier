...
fn conditional_program() {
    ...
    let mut program_hash = [0, 0, 0, 0];
    let step = traverse(program.root().body(), &mut vec![1], &mut program_hash, 0);
    let step = close_block(&mut program_hash, 0, 0, true, step);
    assert_eq!(*program.hash(), hash_to_bytes(&program_hash));
    assert_eq!(63, step);
    ...
    let mut program_hash = [0, 0, 0, 0];
    let step = traverse(program.root().body(), &mut vec![0], &mut program_hash, 0);
    let step = close_block(&mut program_hash, 0, 0, true, step);
    assert_eq!(*program.hash(), hash_to_bytes(&program_hash));
    assert_eq!(63, step);
}
...
fn simple_loop() {
    ...
    // loop executed once
    let mut program_hash = [0, 0, 0, 0];
    let step = traverse(program.root().body(), &mut vec![0, 1], &mut program_hash, 0);
    let step = close_block(&mut program_hash, 0, 0, true, step);
    assert_eq!(*program.hash(), hash_to_bytes(&program_hash));
    assert_eq!(79, step);
