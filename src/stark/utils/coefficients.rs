use crate::{
    math::field,
    stark::constraints::{NUM_AUX_STACK_CONSTRAINTS, NUM_STATIC_DECODER_CONSTRAINTS},
    utils::RangeSlider,
    MAX_CONTEXT_DEPTH, MAX_LOOP_DEPTH, MAX_PUBLIC_INPUTS, MAX_REGISTER_COUNT, MAX_STACK_DEPTH,
    MIN_CONTEXT_DEPTH, MIN_LOOP_DEPTH, MIN_STACK_DEPTH, NUM_CF_OP_BITS, NUM_HD_OP_BITS,
    NUM_LD_OP_BITS, SPONGE_WIDTH,
};
...
const NUM_BOUNDARY_CONSTRAINTS: usize = 1   // for op_counter
    + SPONGE_WIDTH
    + NUM_OP_BITS
    + MAX_CONTEXT_DEPTH
    + MAX_LOOP_DEPTH
...
impl ConstraintCoefficients {
    pub fn new(
        ...
        let transition =
            build_transition_coefficients(&coefficients[i..], ctx_depth, loop_depth, stack_depth);
...
impl CompositionCoefficients {
    pub fn new(seed: [u8; 32]) -> CompositionCoefficients {
        ...
        return CompositionCoefficients {
            trace1,
            trace2,
            t1_degree,
            t2_degree,
...
fn build_boundary_coefficients(coefficients: &[u128]) -> (BoundaryCoefficients, usize) {
    ...
    result
        .op_counter
        .copy_from_slice(&coefficients[range.clone()]);
...
fn build_transition_coefficients(
    coefficients: &[u128],
    ctx_depth: usize,
    loop_depth: usize,
    stack_depth: usize,
    ...
    s_range = new_range(
        s_range.start + MAX_LOOP_DEPTH * 2,
