...
impl ProofOptions {
    pub fn new(
        ...
        assert!(
            ...
            "extension_factor must be a power of 2"
        );
        assert!(
            extension_factor >= 16,
            "extension_factor cannot be smaller than 16"
        );
        assert!(
            extension_factor <= 256,
            "extension_factor cannot be greater than 256"
        );
    ...
    pub fn security_level(&self, optimistic: bool) -> u32 {
        ...
        let num_queries = if optimistic == true {
            self.num_queries
        } else {
            self.num_queries / 2
        };
