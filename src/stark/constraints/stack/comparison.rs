...
pub fn enforce_eq(
    ...
    aux.agg_constraint(0, op_flag, field::mul(new_stack[0], diff));
}
...
pub fn enforce_cmp(result: &mut [u128], old_stack: &[u128], new_stack: &[u128], op_flag: u128) {
    ...
    let gt = field::add(old_stack[GT_IDX], field::mul(bit_gt, not_set));
    let lt = field::add(old_stack[LT_IDX], field::mul(bit_lt, not_set));
    ...
    let x_acc = field::add(old_stack[X_ACC_IDX], field::mul(x_bit, power_of_two));
    let y_acc = field::add(old_stack[Y_ACC_IDX], field::mul(y_bit, power_of_two));
    ...
    let not_set_check = field::mul(binary_not(old_stack[LT_IDX]), binary_not(old_stack[GT_IDX]));
    result.agg_constraint(6, op_flag, are_equal(not_set, not_set_check));

    // power of 2 register was updated correctly
    let power_of_two_constraint = are_equal(field::mul(new_stack[POW2_IDX], 2), power_of_two);
