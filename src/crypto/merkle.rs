...
mod tests {
    ...
    static LEAVES4: [[u8; 32]; 4] = [
        [
            166, 168, 47, 140, 153, 86, 156, 86, 226, 229, 149, 76, 70, 132, 209, 109, 166, 193,
            113, 197, 42, 116, 170, 144, 74, 104, 29, 110, 220, 49, 224, 123,
        ],
        [
            243, 57, 40, 140, 185, 79, 188, 229, 232, 117, 143, 118, 235, 229, 73, 251, 163, 246,
            151, 170, 14, 243, 255, 127, 175, 230, 94, 227, 214, 5, 89, 105,
        ],
        [
        ...
        [
            47, 173, 224, 232, 30, 46, 197, 186, 215, 15, 134, 211, 73, 14, 34, 216, 6, 11, 217,
            150, 90, 242, 8, 31, 73, 85, 150, 254, 229, 244, 23, 231,
        ],
    ];

    static LEAVES8: [[u8; 32]; 8] = [
        [
            115, 29, 176, 48, 97, 18, 34, 142, 51, 18, 164, 235, 236, 96, 113, 132, 189, 26, 70,
            93, 101, 143, 142, 52, 252, 33, 80, 157, 194, 52, 209, 129,
        ],
        [
            52, 46, 37, 214, 24, 248, 121, 199, 229, 25, 171, 67, 65, 37, 98, 142, 182, 72, 202,
            42, 223, 160, 136, 60, 38, 255, 222, 82, 26, 27, 130, 203,
        ],
        [
            130, 43, 231, 0, 59, 228, 152, 140, 18, 33, 87, 27, 49, 190, 44, 82, 188, 155, 163,
            108, 166, 198, 106, 143, 83, 167, 201, 152, 106, 176, 242, 119,
        ],
        [
            207, 158, 56, 143, 28, 146, 238, 47, 169, 32, 166, 97, 163, 238, 171, 243, 33, 209,
            120, 219, 17, 182, 96, 136, 13, 90, 6, 27, 247, 242, 49, 111,
        ],
        [
            179, 64, 123, 119, 226, 139, 161, 127, 36, 251, 218, 88, 20, 217, 212, 85, 112, 85,
            185, 193, 230, 181, 4, 22, 54, 219, 135, 98, 235, 180, 182, 7,
        ],
        [
            101, 240, 19, 44, 43, 213, 31, 138, 39, 26, 82, 147, 255, 96, 234, 51, 105, 6, 233,
            144, 255, 187, 242, 3, 157, 246, 55, 175, 98, 121, 92, 175,
        ],
        [
        ...
        [
            247, 43, 130, 141, 234, 172, 61, 187, 109, 31, 56, 30, 14, 232, 92, 158, 48, 161, 108,
            234, 170, 180, 233, 77, 200, 248, 45, 152, 125, 11, 1, 171,
        ],
    ];
    ...
    fn verify() {
        ...
        assert_eq!(
            true,
            super::MerkleTree::verify(tree.root(), 1, &proof, hash::poseidon)
        );

        let proof = tree.prove(2);
        assert_eq!(
            true,
            super::MerkleTree::verify(tree.root(), 2, &proof, hash::poseidon)
        );
        ...
        assert_eq!(
            true,
            super::MerkleTree::verify(tree.root(), 6, &proof, hash::poseidon)
        );
    }
    ...
    fn verify_batch() {
        ...
        assert_eq!(
            true,
            super::MerkleTree::verify_batch(tree.root(), &[1], &proof, hash::poseidon)
        );
        assert_eq!(
            false,
            super::MerkleTree::verify_batch(tree.root(), &[2], &proof, hash::poseidon)
        );

        let proof = tree.prove_batch(&[1, 2]);
        assert_eq!(
            true,
            super::MerkleTree::verify_batch(tree.root(), &[1, 2], &proof, hash::poseidon)
        );
        assert_eq!(
            false,
            super::MerkleTree::verify_batch(tree.root(), &[1], &proof, hash::poseidon)
        ...
        assert_eq!(
            false,
            super::MerkleTree::verify_batch(tree.root(), &[1, 3], &proof, hash::poseidon)
        );
        assert_eq!(
            false,
            super::MerkleTree::verify_batch(tree.root(), &[1, 2, 3], &proof, hash::poseidon)
        );

        let proof = tree.prove_batch(&[1, 6]);
        assert_eq!(
            true,
            super::MerkleTree::verify_batch(tree.root(), &[1, 6], &proof, hash::poseidon)
        );

        let proof = tree.prove_batch(&[1, 3, 6]);
        assert_eq!(
            true,
            super::MerkleTree::verify_batch(tree.root(), &[1, 3, 6], &proof, hash::poseidon)
        );
